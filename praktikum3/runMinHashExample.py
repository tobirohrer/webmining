...
randomSeed = int(input("randomSeed > "))
...
numHashes = int(input("numHashes > "))

# You can run this code for different portions of the dataset.
# It ships with data set sizes 100, 1000, 2500, and 10000.
numDocs = 1000
dataFile = "./runMinHashData/articles_" + str(numDocs) + ".train"
truthFile = "./runMinHashData/articles_" + str(numDocs) + ".truth"
...
numberShingles = int(input("numberShingles > "))
mode = input("Word (w) or Character (c)? > ")
...
for i in range(0, numDocs):
    ...
    if mode == 'c':
        for index in range(0, len(line) - numberShingles):
            # for index in range(0, len(line) - 4):
            # Construct the shingle text by combining three words together.
            #shingle = words[index] + " " + words[index + 1] + " " + words[index + 2]
            # shingle = line[index : index+4]
            shingle = line[index]
            for i in range(1, numberShingles):
                shingle += line[index+i]
    ...
    if mode == 'w':
        for index in range(0, len(words) - numberShingles):
            # for index in range(0, len(line) - 4):
            # Construct the shingle text by combining three words together.
            #shingle = words[index] + " " + words[index + 1] + " " + words[index + 2]
            # shingle = line[index : index+4]
    ...
    if mode == 'c':
        totalShingles = totalShingles + (len(line) - numberShingles)
    if mode == 'w':
        totalShingles = totalShingles + (len(words) - numberShingles)
